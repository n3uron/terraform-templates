resource "aws_vpc" "n3uron_vpc" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "At least two distinct availability zones are required in the selected region/profile."
    }
  }
...
resource "aws_subnet" "n3uron_subnet_primary" {
  count = var.existing_vpc_id == null ? 1 : 0
...
resource "aws_subnet" "n3uron_subnet_backup" {
  count = var.existing_vpc_id == null ? 1 : 0
  ...
  tags = {
    Name = "${var.name_prefix}-public-subnet-backup"
    Role = "backup"
  }
}
...
resource "aws_route_table_association" "n3uron_rt_assoc_primary" {
  count = var.existing_vpc_id == null ? 1 : 0

  subnet_id      = aws_subnet.n3uron_subnet_primary[0].id
  route_table_id = aws_route_table.n3uron_rt[0].id
}

resource "aws_route_table_association" "n3uron_rt_assoc_backup" {
  count = var.existing_vpc_id == null ? 1 : 0

  subnet_id      = aws_subnet.n3uron_subnet_backup[0].id
  route_table_id = aws_route_table.n3uron_rt[0].id
}
...
resource "aws_security_group" "n3uron_sg" {
  ...
  description = "Security group for redundant EC2 instances"
  vpc_id      = local.vpc_id
...
resource "aws_security_group" "backup_client" {
  name_prefix = "${var.name_prefix}-backup-client-sg-"
  description = "Backup instance client security group"
  vpc_id      = local.vpc_id
  ...
  tags = { Name = "${var.name_prefix}-backup-client-sg" }
}

resource "aws_security_group" "n3uron_redundancy" {
  name_prefix = "${var.name_prefix}-n3uron-redundancy-sg-"
  description = "N3uron redundancy traffic from backup to primary (TCP 3002)"
  vpc_id      = local.vpc_id

  ingress {
    description     = "N3uron redundancy from backup"
    from_port       = 3002
    to_port         = 3002
    protocol        = "tcp"
    security_groups = [aws_security_group.backup_client.id]
  }

  tags = { Name = "${var.name_prefix}-n3uron-redundancy-sg" }
}

resource "aws_route" "atlas_peering" {
  count                     = var.atlas_vpc_peering && var.existing_vpc_id == null ? 1 : 0
  route_table_id            = aws_route_table.n3uron_rt[0].id
